...
export function asAsync<TProps>(options: IAsAsyncOptions<TProps>) {
  ...
  > {
    public state = {
      Component: _syncModuleCache
        ? (_syncModuleCache.get(options.load) as React.ElementType<TProps>)
        : undefined
    };
    ...
    public render(): JSX.Element | null {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { forwardedRef, asyncPlaceholder: Placeholder, ...rest } = this
        .props as any;
        ...
        options
          ...
          .then((LoadedComponent: React.ElementType<TProps>) => {
            if (LoadedComponent) {
              ...
              _syncModuleCache &&
                _syncModuleCache.set(options.load, LoadedComponent);
  ...
  return React.forwardRef<
    ...
    TProps & { asyncPlaceholder?: React.ElementType }
