...
export class EventGroup {
  ...
  public static raise(
    ...
    if (EventGroup._isElement(target)) {
      if (typeof document !== 'undefined' && document.createEvent) {
        ...
        (document as any).createEventObject
      ) {
        // IE8
  ...
  public static isDeclared(target: any, eventName: string): boolean {
    let declaredEvents =
      target && <IDeclaredEventsByName>target.__declaredEvents;
  ...
  public on(
    ...
    if (eventName.indexOf(',') > -1) {
      ...
      let events = <IEventRecordsByName>(
      ...
      if (EventGroup._isElement(target)) {
        ...
        if (target.addEventListener) {
          (<EventTarget>target).addEventListener(
            eventName,
            processElementEvent,
            options
          );
  ...
  public off(
    ...
    for (let i = 0; i < this._eventRecords.length; i++) {
      ...
      if (
        ...
        let targetArray = targetArrayLookup
          ? <IEventRecord[]>targetArrayLookup[this._id]
          : null;
  ...
  public declare(event: string | string[]): void {
    let declaredEvents = (this._parent.__declaredEvents =
