...
export function modalize(
  ...
  const blacklist = [target, ...exclude];
  // start at target, then recurse and do the same for parent, until we reach <body>
  while (target !== targetDocument.body) {
    // grab all siblings of current element
    for (const sibling of (target.parentElement!.children as unknown) as Array<
      HTMLElement
    >) {
      // but ignore elements that are already aria-hidden

      if (
        blacklist.indexOf(sibling) === -1 &&
        blacklistTagNames.indexOf(sibling.tagName) === -1 &&
...
function unmodalize(affectedNodes: HTMLElement[]) {
  affectedNodes.forEach((node) => {
    if (blacklistTagNames.indexOf(node.tagName) === -1) {
