 ...
 */
export function portalContainsElement(
  target: HTMLElement,
  parent?: HTMLElement
): boolean {
  const elementMatch = findElementRecursive(
    ...
    (testElement: HTMLElement) =>
      parent === testElement || testElement.hasAttribute(DATA_PORTAL_ATTRIBUTE)
  );
  return (
    elementMatch !== null && elementMatch.hasAttribute(DATA_PORTAL_ATTRIBUTE)
  );
}
