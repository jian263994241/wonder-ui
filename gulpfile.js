...
var paths = {
  scripts: {
    ...
    icons: {
      ...
      copylist: ['packages/icons/package.json', 'packages/icons/README.md'],
    },
    router: {
    ...
    utils: {
      ...
      copylist: ['packages/utils/package.json', 'packages/utils/README.md'],
    },
    styles: {
...
function createTask(target) {
  return gulp.series(
    function cpoyInfo() {
      return gulp
        .src(paths.scripts[target].copylist)
        .pipe(gulp.dest(paths.scripts[target].dest));
    },
    function docgen() {
      return gulp.src(paths.scripts[target].src, { sourcemaps: false }).pipe(
    ...
    function scripts() {
      return gulp
        .src(paths.scripts[target].src, { sourcemaps: false })
        .pipe(
          babel({
            presets: [preset],
          }),
